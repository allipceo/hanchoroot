...
function convertAllNotionData() {
  ...
  const convertedPersons = notionData.results.map((person, index) => {
    ...
    if (index % 10 === 0) {
      console.log(`변환 진행: ${index + 1}/${notionData.results.length} - ${converted.name}`);
    }
  ...
  // 결과 저장
  fs.writeFileSync('converted_full_data.json', JSON.stringify(result, null, 2), 'utf8');
  
  console.log(`\n전체 변환 완료!`);
  console.log(`- 총 ${convertedPersons.length}명 변환`);
  console.log(`- 결과 파일: converted_full_data.json`);
  console.log(`- 파일 크기: ${fs.statSync('converted_full_data.json').size} bytes`);
