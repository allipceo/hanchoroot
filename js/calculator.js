...
class ChonsuCalculatorUI {
    ...
    populatePersonSelects() {
        ...
        joFamily.forEach(person => {
            ...
            option1.textContent = `${person.name} ${/-M-/.test(person.id)?'(M)':(/-F-/.test(person.id)?'(F)':'')} (${generation1}세대)`;
            select1.appendChild(option1);

            const option2 = document.createElement('option');
            option2.value = person.id;
            const generation2 = person.id ? person.id.split('-')[1]?.substring(1) || person.세대 : person.세대;
            option2.textContent = `${person.name} ${/-M-/.test(person.id)?'(M)':(/-F-/.test(person.id)?'(F)':'')} (${generation2}세대)`;
